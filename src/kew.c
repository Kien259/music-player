...
struct Event processInput()
{
        ...
        if (appState.currentView == WEB_SEARCH_VIEW || appState.currentView == ALBUM_SEARCH_VIEW)
        {
                if (strcmp(event.key, "\x7F") == 0 || strcmp(event.key, "\x08") == 0)
                {
                        removeFromWebSearchQuery();
                }
                else if (strcmp(event.key, "\n") == 0 || strcmp(event.key, "\r") == 0)
                {
                        manualSearch();
                }
                else if (((strnlen(event.key, sizeof(event.key)) == 1 && event.key[0] != '\033' && event.key[0] != '\n' && event.key[0] != '\t' && event.key[0] != '\r') || strcmp(event.key, " ") == 0 || (unsigned char)event.key[0] >= 0xC0) && strcmp(event.key, "Z") != 0 && strcmp(event.key, "X") != 0 && strcmp(event.key, "C") != 0 && strcmp(event.key, "V") != 0 && strcmp(event.key, "B") != 0 && strcmp(event.key, "N") != 0)
                {
                        addToWebSearchQuery(event.key);
                }
        }
...
void handleInput(AppState *state)
{
        ...
        case EVENT_SHOWSEARCH:
                ...
                break;
        case EVENT_SHOWWEBSEARCH:
                toggleShowView(WEB_SEARCH_VIEW);
                if (appState.currentView == WEB_SEARCH_VIEW) {
                        initWebSearchUI();
                }
                break;
        case EVENT_SHOWALBHUMSEARCH:
                toggleShowView(ALBUM_SEARCH_VIEW);
                if (appState.currentView == ALBUM_SEARCH_VIEW) {
                        initWebSearchUI();
                }
                break;
        case EVENT_DOWNLOADMUSIC:
                if (appState.currentView == WEB_SEARCH_VIEW || appState.currentView == ALBUM_SEARCH_VIEW) {
                        downloadCurrentSelection();
                }
                break;
        case EVENT_WEBSEARCH_SEARCH:
                if (appState.currentView == WEB_SEARCH_VIEW || appState.currentView == ALBUM_SEARCH_VIEW) {
                        manualSearch();
                }
                break;
