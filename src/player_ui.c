...
void printHelp()
{
        printf(" kew - A terminal music player.\n"
               ...
               " F7 to show/hide album search view.\n"
               " F8 to show/hide track search view.\n"
               " F6 to show/hide show/hide key bindings view.\n"
               " F7 to show/hide web search view.\n"
...
void switchToNextView(void)
{
        ...
        case TRACK_VIEW:
                ...
                break;
        case SEARCH_VIEW:
                appState.currentView = ALBUM_SEARCH_VIEW;
                break;
        case ALBUM_SEARCH_VIEW:
                appState.currentView = WEB_SEARCH_VIEW;
                break;
        case WEB_SEARCH_VIEW:
...
void flipNextPage(void)
{
        ...
        {
                currentWebSearchRow++;
                int maxRow = getWebSearchResultsCount() - 1;
                if (maxRow >= 0 && currentWebSearchRow > maxRow) {
                        currentWebSearchRow = maxRow;
                }
                refresh = true;
        }
        else if (appState.currentView == ALBUM_SEARCH_VIEW)
        {
                currentWebSearchRow++;
                int maxRow = getWebSearchResultsCount() - 1;
                if (maxRow >= 0 && currentWebSearchRow > maxRow) {
...
int printPlayer(SongData *songdata, double elapsedSeconds, AppSettings *settings, AppState *state)
{
        ...
        else if (state->currentView == WEB_SEARCH_VIEW && refresh)
        {
                clearScreen();
                showWebSearch(settings, ui);
                refresh = false;
                fflush(stdout);
        }
        else if (state->currentView == ALBUM_SEARCH_VIEW && refresh)
        {
                clearScreen();
                showAlbumSearch(settings, ui);
                refresh = false;
                fflush(stdout);
        }
