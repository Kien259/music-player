...
AppSettings constructAppSettings(KeyValuePair *pairs, int count)
{
        ...
        c_strcpy(settings.mouseLeftClick, "[<0", sizeof(settings.mouseLeftClick));
        c_strcpy(settings.mouseMiddleClick, "[<1", sizeof(settings.mouseMiddleClick));
        c_strcpy(settings.mouseRightClick, "[<2", sizeof(settings.mouseRightClick));
        c_strcpy(settings.mouseScrollUp, "[<64", sizeof(settings.mouseScrollUp));
        c_strcpy(settings.mouseScrollDown, "[<65", sizeof(settings.mouseScrollDown));
        c_strcpy(settings.mouseAltScrollUp, "[<72", sizeof(settings.mouseAltScrollUp));
        c_strcpy(settings.mouseAltScrollDown, "[<73", sizeof(settings.mouseAltScrollDown));
        ...
        c_strcpy(settings.downloadMusic, "\x04", sizeof(settings.downloadMusic));
        ...
        for (int i = 0; i < count; i++)
        {
                ...
                else if (strcmp(lowercaseKey, "hardshowwebsearch") == 0)
                {
                        if (strcmp(pair->value, "") != 0)
                                snprintf(settings.hardShowWebSearch, sizeof(settings.hardShowWebSearch), "%s", pair->value);
                }
                else if (strcmp(lowercaseKey, "togglealbumstracks") == 0)
                {
                        if (strcmp(pair->value, "") != 0)
                                snprintf(settings.toggleAlbumsTracks, sizeof(settings.toggleAlbumsTracks), "%s", pair->value);
                }
                else if (strcmp(lowercaseKey, "downloadmusic") == 0)
                {
                        if (strcmp(pair->value, "") != 0)
                                snprintf(settings.downloadMusic, sizeof(settings.downloadMusic), "%s", pair->value);
                }
...
void mapSettingsToKeys(AppSettings *settings, UISettings *ui, EventMapping *mappings)
{
        ...
        mappings[61] = (EventMapping){settings->hardShowWebSearch, EVENT_SHOWWEBSEARCH};
        mappings[62] = (EventMapping){settings->hardShowAlbumSearch, EVENT_SHOWALBHUMSEARCH};
        mappings[63] = (EventMapping){settings->downloadMusic, EVENT_DOWNLOADMUSIC};
}
...
void setConfig(AppSettings *settings, UISettings *ui)
{
        ...
        fprintf(file, "hardShowWebSearch=%s\n", settings->hardShowWebSearch);
        fprintf(file, "hardShowAlbumSearch=%s\n", settings->hardShowAlbumSearch);
        fprintf(file, "toggleAlbumsTracks=%s\n", settings->toggleAlbumsTracks);
        fprintf(file, "downloadMusic=%s\n", settings->downloadMusic);
        fprintf(file, "quit=%s\n", settings->quit);
